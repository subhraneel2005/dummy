...
export function ClaudePermission({
  ...
  return (
    ...
    >
      ...
      <div className="text-[var(--foreground)]">{command}</div>
      <div className="mb-1.5 mt-2 text-[var(--foreground)]">{question}</div>
        ...
        {options.map((opt, i) => {
          ...
          return (
            <div
              ...
              className={`flex cursor-pointer items-baseline gap-2 rounded px-1 py-0.5 outline-none focus-visible:ring-1 focus-visible:ring-ring/60 ${
                active ? "bg-accent text-accent-foreground" : "text-transparent"
              }`}
            >
              <span
                aria-hidden
                className={active ? "" : "text-transparent"}
                style={{ width: "1ch" }}
              >
                ❯
              </span>
